...
const SuppliersTypeList = () => {
  const history = useHistory()
  const dispatch = useDispatch();
  const supplierTypesList = useSelector(state => state.suppliers.supplierTypesList);
  useEffect(() => {
    dispatch(Actions.getSupplierTypes());
  }, [])

  /* tabla */
  const [supplierTypesList_formated,updateList] = useState([]);
  useEffect(() => {//builds a list of objects with new key names
    const formatedList = supplierTypesList.map( client => {
      let formatedObj = {
        Código: client.codigo_Tipo,
        'Nombre Tipo': client.nombre_Tipo,
      }
      return formatedObj;
    });
  ...
  const fields = [
    ...
    { key: 'Nombre Tipo', _style: { width: '45%'} },
    {
      key: 'show_details',
      label: 'Acciones',
      _style: { width: '10%' },
      ...
      <CCol xl={12}>
        <CCard>
          <CCardHeader>
            ...
            <h1 style={{fontWeight:'bold', lineHeight:'35px', fontSize:'45px',textAlign:'center',textDecoration:'underline'}}>Tipos de Proveedores Registrados</h1>
              <ReactHTMLTableToExcel 
                id="btn test"
                className="btn btn-success"
                table="registry_Typesuppliers"
                filename="Tipo de proveedores registrados"
                sheet="pagina 1"
          ...
          <CDataTable
                ...
                // id="disabledInput"
                items={supplierTypesList_formated}
                fields={fields}
              ...
              />
              <hr />
            {/* <table id="registry_Typesuppliers" className="table table-striped">
            <thead>
              <tr>
          ...
          <CButton className="btn_registry--lines" type="submit" color="btn btn-info" variant="outline" onClick={() => {
                history.push('/proveedoresTipo-registro')
              }}>Registrar Tipo Proveedor</CButton>
          </CCardBody>
        </CCard>
      </CCol>
    </CRow>
...
export default SuppliersTypeList
